<template>
  <div class="min-h-screen bg-dark text-light flex items-center justify-center p-4">
    ...
    <div class="relative z-10 w-full max-w-md">
      <div class="backdrop-blur-custom border border-dark-lighter rounded-2xl p-8 shadow-2xl">
        <!-- Header -->
        <div class="text-center mb-8">
          <h1 class="text-heading-2 text-light mb-2">{{ $t('auth.welcome') }}</h1>
        ...
        <div class="flex bg-dark-lighter rounded-xl p-1 mb-6">
          <button 
            @click="isLogin = true"
            class="flex-1 py-2 px-4 text-sm font-medium rounded-lg transition-default"
        ...
        <form @submit.prevent="handleSubmit" class="space-y-4">
          ...
          <button 
            type="submit"
            :disabled="loading"
            class="btn-primary w-full py-3 font-medium disabled:opacity-50 disabled:cursor-not-allowed"
          >
            <span v-if="loading" class="flex items-center justify-center">
              <Icon icon="tabler:loader-2" class="loading-spinner mr-2" />
              {{ $t('auth.processing') }}
            </span>
        ...
        <!-- Social login -->
        <div class="space-y-3">
          <button class="btn-secondary w-full py-3 flex items-center justify-center">
