export default {
  nav: {
    home: "首页",
    tools: "工具",
    features: "特性",
    about: "关于我们",
    testimonials: "用户评价",
    price: "价格",
    tryFree: "注册/登录",
    welcome: "欢迎",
    logout: "退出登录"
  },
  hero: {
    title: "专业环境消毒灭菌服务",
    subtitle: "守护健康人居环境",
    description: "森德优护（北京）科技公司，清华科技园孵化的科技创新企业，专注于人居环境有害微生物污染治理，采用源自瑞士SANOSIL的先进灭菌技术。",
    tryFree: "免费咨询服务",
    viewDemo: "查看服务详情",
    welcome_back: "欢迎回来"
  },
  auth: {
    welcome: "欢迎使用 WebAI",
    login_subtitle: "登录您的账户",
    register_subtitle: "创建新账户",
    login: "登录",
    register: "注册",
    name: "姓名",
    name_placeholder: "请输入您的姓名",
    email: "邮箱",
    email_placeholder: "请输入您的邮箱",
    password: "密码",
    password_placeholder: "请输入您的密码",
    confirm_password: "确认密码",
    confirm_password_placeholder: "请再次输入密码",
    processing: "处理中...",
    or_divider: "或",
    google_continue: "使用 Google 继续",
    github_continue: "使用 GitHub 继续",
    back_to_home: "← 返回首页",
    password_mismatch: "密码不一致",
    login_success: "登录成功",
    register_success: "注册成功"
  },
  partners: {
    title: "专业资质认证",
    subtitle: "中关村高新企业 | 公共环境消毒杀菌企业甲级资质",
    member_title: "协会会员单位",
    beijing_association: "北京市室内及车内环境净化协会优秀会员单位",
    shanghai_association: "上海市环境净化行业协会优秀会员单位",
    tech_innovation: "科技创新认证企业",
    certification_badge: "认",
    grade_badge: "甲",
    grade_qualification: "甲级资质"
  },
  features: {
    title: "专业环境卫生健康服务",
    subtitle: "致力于环境卫生健康事业，降低人类感染风险",
    advanced_tech: "先进灭菌技术",
    advanced_tech_desc: "采用源自瑞士SANOSIL的灭菌技术，高效去除室内病毒病菌污染危害。",
    comprehensive_service: "全面服务体系",
    comprehensive_service_desc: "提供环境消毒、卫生教育、咨询和解决方案的一体化服务。",
    health_environment: "健康环境塑造",
    health_environment_desc: "通过环境消毒控制污染源，塑造健康人居和工作环境。"
  },
  tools: {
    title: "中元氢杰卫生服务项目",
    subtitle: "森德优护（北京）科技公司专业消毒灭菌服务",
    home_disinfection: "家庭消毒",
    home_disinfection_desc: "专业的家庭环境消毒灭菌服务，保障家人健康安全。",
    office_disinfection: "办公场所消毒",
    office_disinfection_desc: "为企业提供安全的工作环境，降低疫情传播风险。",
    car_disinfection: "车内消毒",
    car_disinfection_desc: "车内环境深度清洁消毒，创造安全的出行空间。",
    health_education: "卫生教育",
    health_education_desc: "提供专业的卫生健康知识培训和指导服务。",
    consultation: "专业咨询",
    consultation_desc: "根据客户需求提供定制化的消毒解决方案。",
    equipment_service: "设备服务",
    equipment_service_desc: "专业消毒设备的安装、维护和技术支持服务。",
    learn_more: "了解更多"
  },
  trending: {
    title: "热门话题",
    subtitle: "发现其他用户在使用 WebAI 创作什么",
    ai_content_generation: "AI内容生成",
    ai_content_generation_desc: "探索AI驱动内容创作的最新趋势。",
    marketing_copy: "营销文案",
    marketing_copy_desc: "了解AI如何改变营销内容创作。",
    seo_optimization: "SEO优化",
    seo_optimization_desc: "发现AI驱动的SEO技术和策略。",
    users: "用户"
  },
  faq: {
    title: "常见问题",
    subtitle: "关于 WebAI 您需要了解的一切",
    q1: "WebAI 如何生成内容？",
    a1: "WebAI 使用先进的机器学习算法和自然语言处理技术，根据您的输入和要求生成高质量内容。",
    q2: "WebAI 可以创建什么类型的内容？",
    a2: "WebAI 可以创建各种类型的内容，包括博客文章、社交媒体帖子、营销文案、产品描述等。",
    q3: "WebAI 生成的内容是独特的吗？",
    a3: "是的，WebAI 生成的所有内容都是独特且原创的。我们的AI模型经过训练，在保持自然语言模式的同时创建新鲜内容。"
  },
  pricing: {
    title: "适合每个人的价格和方案",
    subtitle: "最佳写作AI替代平台，轻松创建内容",
    free: "免费版",
    starter: "初级版",
    basic: "基础版",
    enterprise: "企业版",
    try_plan: "试用",
    price_per_month: "/月",
    features: {
      words_per_month: "字数/月",
      words_5k: "5,000字",
      words_50k: "50,000字",
      words_200k: "200,000字",
      words_unlimited: "无限字数",
      basic_templates: "基础模板",
      all_templates: "所有模板",
      custom_templates: "自定义模板",
      custom_solutions: "定制解决方案",
      standard_support: "标准支持",
      priority_support: "优先支持",
      support_24_7: "24/7支持",
      dedicated_support: "专属支持",
      single_user: "单用户",
      users_3: "最多3个用户",
      users_10: "最多10个用户",
      users_unlimited: "无限用户"
    }
  },
  about: {
    title: "关于森德优护",
    subtitle: "清华科技园孵化的科技创新企业",
    company_title: "公司简介",
    company_intro: "森德优护（北京）科技公司致力于环境卫生健康事业，专注于人居环境有害微生物污染治理。通过环境消毒控制污染源及病毒病菌污染，降低人类感染风险，塑造健康人居和工作环境。",
    project_title: "服务项目",
    project_intro: "中元氢杰卫生服务项目是森德优护（北京）科技公司推出的家庭消毒灭菌服务项目，采用源自瑞士SANOSIL的灭菌技术，提供去除室内病毒病菌污染危害服务的同时，也提供卫生教育、咨询、解决方案服务。",
    qualifications_title: "企业资质",
    high_tech: "中关村高新企业单位",
    disinfection_qualification: "公共环境消毒杀菌企业甲级资质",
    contact_us: "联系我们"
  },
  footer: {
    title: "森德优护（北京）科技公司",
    subtitle: "专业环境消毒灭菌服务提供商",
    contact_info: "联系方式",
    phone: "电话：13911874044（雁西）",
    email: "邮箱：476341210{'@'}qq.com",
    copyright: "© 2024 森德优护（北京）科技公司. 版权所有。"
  },
  apiDocs: {
    title: "API 文档",
    description: "查看完整的 API 接口文档，了解如何与我们的平台进行集成"
  },
  dashboard: {
    title: "控制台",
    welcome_message: "欢迎来到您的个人控制台，开始您的AI之旅",
    ai_assistant: "AI消毒助手",
    ai_assistant_desc: "专业微生物咨询与消毒技术指导",
    ai_instructor: "AI消毒金牌讲师",
    ai_instructor_desc: "专业的网络安全培训和指导",
    my_profile: "我的",
    profile_desc: "管理个人信息和系统设置",
    virus_scan: "病毒扫描",
    virus_scan_desc: "全面的系统安全检测",
    start_scan: "开始扫描",
    file_analysis: "文件分析",
    file_analysis_desc: "上传文件进行安全分析",
    upload_file: "上传文件",
    recent_scans: "最近扫描",
    system_scan: "系统扫描",
    hours_ago: "小时前",
    day_ago: "天前",
    security_courses: "安全课程",
    security_courses_desc: "学习网络安全知识",
    view_courses: "查看课程",
    video_tutorials: "视频教程",
    video_tutorials_desc: "观看专业安全教程",
    watch_videos: "观看视频",
    ai_chat: "AI对话",
    ai_chat_desc: "与AI安全专家对话",
    start_chat: "开始对话",
    personal_info: "个人信息",
    system_settings: "系统设置",
    language: "语言",
    theme: "主题",
    notifications: "通知设置",
    email_notifications: "邮件通知",
    security_alerts: "安全警报",
    save_changes: "保存更改",
    chat_welcome: "您好！我是您的专业消毒灭菌顾问，拥有丰富的微生物学和消毒技术知识。\n\n无论您是家庭用户寻求日常消毒建议，还是专业人士需要深度技术支持，我都能为您提供精准、实用的解决方案。我的专长包括:\n\n• 微生物防控与消毒灭菌技术\n• 不同场景的消毒方案定制\n• 专业消毒产品推荐与使用指导\n• 特殊环境消毒难题解决\n\n我会根据您的具体情况和专业背景，调整回答的深度和专业程度，确保您获得最适合的建议。\n\n请问您是:\n1️⃣ 普通消费者（家庭日常消毒）\n2️⃣ 行业从业人员（从事消毒相关工作）\n3️⃣ 企业管理者（需要制定消毒方案）\n4️⃣ 医学专业人士（临床或研究应用）\n\n您选择哪个数字呢？我将为您提供量身定制的专业服务！",
    quick_scan: "1️⃣ 普通消费者（家庭日常消毒）",
    upload_analysis: "2️⃣ 行业从业人员（从事消毒相关工作）",
    security_tips: "3️⃣ 企业管理者（需要制定消毒方案）",
    medical_professional: "4️⃣ 医学专业人士（临床或研究应用）",
    type_message: "输入您的问题...",
    voice_record: "语音输入",
    voice_play: "播放语音",
    voice_recording: "正在录音...",
    voice_processing: "正在处理语音...",
    voice_error: "语音处理失败",
    ai_scan_response: "正在为您启动系统扫描，请稍候。扫描将检查系统中的恶意软件、病毒和潜在威胁。",
    ai_file_response: "请上传您需要分析的文件，我将对其进行全面的安全检测，包括病毒扫描和恶意代码分析。",
    ai_security_response: "以下是一些重要的安全建议：1. 定期更新系统和软件 2. 使用强密码 3. 不要点击可疑链接 4. 定期备份重要数据",
    ai_default_response: "我理解您的问题。作为AI消毒助手，我可以帮助您进行病毒扫描、文件安全分析和提供网络安全建议。请告诉我您具体需要什么帮助？"
  }
} 